...
usda <- read.csv('../data/historical/yields/barley_all.csv')
...
# Climate data
loca.predictors <- read.csv('../data/historical/predictors/Livneh/Livneh_barley_H21_predictors.csv')
...
# Need to fix later
loca.covariates <- drop_na(loca.covariates, barley_irr_frac)
...
colnames(fips) <- 'fips'
write.csv(fips, file='../data/historical/crop_fips_historical/barley_fips.csv',
          row.names = FALSE)
...
covariates <- subset(loca.covariates, select=append(bios, c('barley_irr_frac', 'fips')))
covariates <- covariates[covariates$fips %in% df$fips,]
covariates <- subset(covariates, select=append(bios, 'barley_irr_frac'))
...
# Store
saveRDS(SR_09_model, file = "../data/simulated/yield_models/barley_SR_09.rds")
...
# Store
saveRDS(SM_ave_model, file = "../data/simulated/yield_models/barley_SM_ave.rds")
...
# Store
saveRDS(SM_week_model, file = "../data/simulated/yield_models/barley_SM_week.rds")
