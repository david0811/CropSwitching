...
usda <- read.csv('../data/historical/yields/sorghum_all.csv')
...
# Climate data
loca.predictors <- read.csv('../data/historical/predictors/Livneh/Livneh_sorghum_H21_predictors.csv')
loca.covariates <- read.csv('../data/historical/predictors/Livneh/Livneh_covariates_all.csv')

# Remove counties with NA irrigation fraction (not producing sorghum circa 2010-2015 when dataset was made)
# Need to fix later
loca.covariates <- drop_na(loca.covariates, sorghum_irr_frac)
...
colnames(fips) <- 'fips'
write.csv(fips, file='../data/historical/crop_fips_historical/sorghum_fips.csv',
          row.names = FALSE)
...
covariates <- subset(loca.covariates, select=append(bios, c('sorghum_irr_frac', 'fips')))
covariates <- covariates[covariates$fips %in% df$fips,]
covariates <- subset(covariates, select=append(bios, 'sorghum_irr_frac'))
...
# Store
saveRDS(SR_09_model, file = "../data/simulated/yield_models/sorghum_SR_09.rds")
...
# Store
saveRDS(SM_ave_model, file = "../data/simulated/yield_models/sorghum_SM_ave.rds")
...
# Store
saveRDS(SM_week_model, file = "../data/simulated/yield_models/sorghum_SM_week.rds")
