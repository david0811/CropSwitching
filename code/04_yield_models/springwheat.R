...
usda <- read.csv('../data/historical/yields/spring_wheat_all.csv')
...
# Climate data
loca.predictors <- read.csv('../data/historical/predictors/Livneh/Livneh_springwheat_H21_predictors.csv')
loca.covariates <- read.csv('../data/historical/predictors/Livneh/Livneh_covariates_all.csv')

# Remove counties with NA irrigation fraction (not producing spring wheat circa 2010-2015 when dataset was made)
# Need to fix later
loca.covariates <- drop_na(loca.covariates, springwheat_irr_frac)
...
colnames(fips) <- 'fips'
write.csv(fips, file='../data/historical/crop_fips_historical/springwheat_fips.csv',
          row.names = FALSE)
...
covariates <- subset(loca.covariates, select=append(bios, c('springwheat_irr_frac', 'fips')))
covariates <- covariates[covariates$fips %in% df$fips,]
covariates <- subset(covariates, select=append(bios, 'springwheat_irr_frac'))
...
# Store
saveRDS(SR_09_model, file = "../data/simulated/yield_models/springwheat_SR_091.rds")
...
# Store
saveRDS(SM_ave_model, file = "../data/simulated/yield_models/springwheat_SM_ave.rds")
...
# Store
saveRDS(SM_week_model, file = "../data/simulated/yield_models/springwheat_SM_week.rds")
